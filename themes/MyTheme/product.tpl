	...
	<div class="primary_block row">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			...
			<p id="product_reference"{if empty($product->reference) || !$product->reference} style="display: none;"{/if}>
				...
				<span class="editable" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}{/if}" id="reference_id">{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span>
				{if (Module::isInstalled('mylabelgenerator'))}
					{hook h='displayHome' mod='mylabelgenerator'}
				{/if}
			</p>
